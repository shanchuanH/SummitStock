...
The read-only market inspection page is available at `http://localhost:4173/market-data`. Deterministic fake adapters are available only under the explicit `local-fixture` and `test` profiles. Real-data analysis can use Alpha Vantage or the explicitly selected Yahoo Finance Chart adapter for market prices, while Alpha Vantage supplies estimates and the shared earnings calendar, SEC EDGAR supplies filings with a declared organization/contact `SEC_USER_AGENT`, and FRED supplies macro evidence. The Yahoo endpoint is unofficial and is therefore recorded in provenance and treated as unavailable whenever its response is incomplete or rejected. Disabled, fake, unavailable, or incomplete production configuration fails closed. `ALLOW_PARTIAL_PRODUCTION=true` is an explicit escape hatch for a non-formal partial deployment; the data-health UI reports `PARTIAL` while any required provider is unavailable.
