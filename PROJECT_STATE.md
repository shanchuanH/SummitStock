...
- Healthy analyst estimates are refreshed from Alpha Vantage only after seven days, safely inside the strategy's 14-day estimate freshness limit. Missing, unhealthy, or stale symbols remain eligible on every daily run, so the optimization never turns absent evidence into a healthy cache hit.
