...
describe("PortfolioListTest", () => {
  ...
  it("shows the required fields in priority order without changing decimal data", async () => {
    renderPage();
    ...
    expect(screen.getByText("ABOVE_TREND")).toBeInTheDocument();
  });
  ...
  it("opens classification from the selected row only", async () => {
    ...
    fetchMock.mockResolvedValueOnce(
      new Response(
        JSON.stringify({
          ...
          source: "SYSTEM_RULE",
          blocked: false,
          reason: "Evidence matches the speculative policy.",
    ...
    await userEvent.click(
      await screen.findByRole("button", { name: "查看系统建议" }),
    );
