...
export function DashboardPage() {
  ...
  const mustAct = actions.data?.mustAct?.slice(0, 3) ?? [];
  return (
    <Frame eyebrow="DAILY CONTROL SURFACE" title="Dashboard">
      <section className="dashboard-grid">
            ...
            <ol className="action-list">
              {mustAct.map((action) => (
                <li key={action.id}>
                  ...
                  <span>{action.action ?? "REVIEW"}</span>
                  <small>{action.confidence ?? "UNKNOWN"}</small>
                </li>
              ))}
            </ol>
          )}
        ...
        <article className="context-card">
          ...
          <p>
            Recommendations are explainable, time-bounded, and manually executed
            outside this system.
          </p>
          <a href="/market-context">Inspect regime and drawdown →</a>
        </article>
        <article className="context-card">
          <h2>Workspace</h2>
          <p>
            {session.data?.authenticated
              ? `Signed in as ${session.data.username ?? "owner"}`
              : "Public market data is available. Portfolio data remains protected."}
          </p>
          <a href="/data-health">Review data and worker health →</a>
        </article>
      </section>
    </Frame>
  );
