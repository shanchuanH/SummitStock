...
public final class CoreEtfDecisionEngine implements AssetDecisionEngine {
    ...
    public List<RecommendationCandidate> evaluate(DecisionContext context) {
        ...
        if ("ETF_DIP_MARKET_DRIVEN".equals(e.drawdown().state())
                ...
                && e.currentWeight().compareTo(context.targetMin()) < 0) {
            values.add(of(
        ...
        } else if (context.targetMin() != null && e.currentWeight().compareTo(context.targetMin()) < 0) {
            values.add(of(
                    RecommendationAction.BUY,
                    "NORMAL",
                    10,
                    "CORE_ETF.ALLOCATION.ADD",
                    "Core allocation is below its policy target with capacity available.",
