...
class ManualAnalysisRunIntegrationTest extends PortfolioImportIntegrationSupport {
    ...
    void authenticatedOwnerCanQueueOneImmediateRerunWithoutConcurrentDuplicates() throws Exception {
        ...
        var firstResponse = mockMvc.perform(post("/api/v1/analysis/runs")
        ...
        assertThat(first.path("state").asString()).isEqualTo("ANALYSIS_QUEUED");
        assertThat(count("SELECT COUNT(*) FROM portfolio_analysis_step WHERE run_id=UUID_TO_BIN('" + rerunId + "')"))
                .isEqualTo(27);

        var repeatedResponse = mockMvc.perform(post("/api/v1/analysis/runs")
                        .with(httpBasic(EMAIL, PASSWORD))
                        .with(csrf()))
                .andReturn();
        ...
        assertThat(repeated.path("state").asString()).isEqualTo("ANALYSIS_RUNNING");
    }
}
