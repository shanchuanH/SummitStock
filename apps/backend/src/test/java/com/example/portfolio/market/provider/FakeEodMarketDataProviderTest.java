...
class FakeEodMarketDataProviderTest {
    ...
    void weekendRangeUsesLatestRequestedTradingSessionForProvenance() {
        ...
        var result = provider.fetchDailyBars(
                "SPY", LocalDate.parse("2026-08-17"), LocalDate.parse("2026-08-22"));

        assertThat(result.bars()).extracting(ProviderModels.DailyBar::marketDate).last()
